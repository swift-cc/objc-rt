...
void objc_invalidate_dtable(Class);
int objc_initialize_loading(FILE *errorStream);
void objc_load_callback(Class, Category *category);
...
struct _object_layout_struct 
{
	unsigned retained;
	char padding[ALIGN - ((UNP % ALIGN) ? (UNP % ALIGN) : ALIGN)];
	// the bytes defined by NSObject follow here
};
typedef _object_layout_struct* _object_layout;
